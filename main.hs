...
printGameBoard gameBoard column = do
    if (length gameBoard) == 0
        then putStr ""
    else do
        let currentCell = (head gameBoard)
        putStr ((show (getCellValue currentCell)) ++ "(" ++ (show (getGroupId currentCell)) ++ ") ")
        if (mod column gameBoardSize) == gameBoardSize - 1
            then putStr "\n"
        else putStr ""
        printGameBoard (tail gameBoard) (snd (getPosition (head (tail gameBoard))))

-- getGroups :: GameBoard -> [Group]

main = do
    let gameBoard = [((0, 0), 1, 1), ((0, 1), 2, 1), ((0, 2), 3, 3), ((0, 3), 4, 3), ((0, 4), 5, 3), ((0, 5), 6, 3), ((0, 6), 7, 3), ((0, 7), 8, 3), ((0, 8), 9, 3), ((0, 9), 10, 3),
                     ...
                     ((8, 0), 81, 2), ((8, 1), 82, 2), ((8, 2), 83, 2), ((8, 3), 84, 3), ((8, 4), 85, 3), ((8, 5), 86, 3), ((8, 6), 87, 3), ((8, 7), 88, 3), ((8, 8), 89, 3), ((8, 9), 90, 3),
